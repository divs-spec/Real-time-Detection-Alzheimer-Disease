...
if test_choice == "Gaze Tracking":
    st.markdown("### Follow the on-screen text with your eyes - This is the paragraph you need to read aloud for the test. It should be large and clearly visible so that gaze tracking works effectively. Try to read it naturally and ensure your eye movements are being tracked.")
    if st.button("Start Gaze Test"):
        gaze_variation = start_gaze_tracking()
        st.markdown(f"*Gaze Movement Variation:* {gaze_variation:.4f} (higher = more movement [higher = Healthy patient])")
